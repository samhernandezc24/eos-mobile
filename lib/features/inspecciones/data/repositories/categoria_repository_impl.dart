...
class CategoriaRepositoryImpl implements CategoriaRepository {
  ...
  @override
  Future<DataState<IReturn>> delete(CategoriaIdParamEntity objData) async {
    try {
      final httpResponse = await _categoriaRemoteApiService.delete(CategoriaIdParamModel.fromEntity(objData));
      if (httpResponse.response.statusCode == HttpStatus.ok) {
        if (httpResponse.data.session ?? false) {
          if (httpResponse.data.action ?? false) {
            return DataSuccess(httpResponse.data);
