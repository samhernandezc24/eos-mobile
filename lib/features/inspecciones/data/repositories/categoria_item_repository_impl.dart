...
class CategoriaItemRepositoryImpl implements CategoriaItemRepository {
  ...
  /// GUARDADO DE DUPLICADO DE CATEGORIA ITEM
  @override
  Future<DataState<ServerResponse>> storeDuplicate(CategoriaItemStoreDuplicateReqEntity objData) async {
    try {
      // Obtener el token localmente.
      final String? token = await authTokenHelper.retrieveRefreshToken();
      ...
      final httpResponse = await _categoriaItemRemoteApiService.storeDuplicate(
        'application/json',
        'Bearer $token',
        CategoriaItemStoreDuplicateReqModel.fromEntity(objData),
      );
  ...
  Future<DataState<ServerResponse>> update(CategoriaItemEntity objData) async {
    try {
      ...
      // Realizar la solicitud usando el token actualizado o el actual.
      final httpResponse = await _categoriaItemRemoteApiService.update('application/json', 'Bearer $token', CategoriaItemModel.fromEntity(objData));
  ...
  Future<DataState<ServerResponse>> delete(CategoriaItemEntity objData) async {
    try {
      ...
      // Realizar la solicitud usando el token actualizado o el actual.
      final httpResponse = await _categoriaItemRemoteApiService.delete('application/json', 'Bearer $token', CategoriaItemModel.fromEntity(objData));
